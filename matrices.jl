...
@generated function compute_residual(series::SeriesModel{N}, statefuns, vars, args) where N
    quote
        R = -SA[values(vars)...]
        Base.@nexprs $N i -> begin # this will be put into a function, hardcoded for now
            R += eval_state_functions(statefuns, series.children[i], args)
        end
        R
    end
end
...
@generated function compute_jacobian(x, series::SeriesModel{N1}, statefuns::NTuple{N2, Any}, args_diff, args_nondiff) where {N1,N2}
    quote
        J = @SMatrix zeros(N2, N2)
        Base.@nexprs $N1 i -> begin # this will be put into a function, hardcoded for now
            J += _compute_jacobian(x, series.children[i], statefuns,  args_diff, args_nondiff)
        end
        J
    end
end
...
@generated function reduce_state_functions(funs::NTuple{N1, Any}, series::SeriesModel{N2}, args::NamedTuple) where {N1, N2}
    quote
        @inline 
        Base.@nexprs $N1 i -> x_i = begin
            val = 0e0
            Base.@nexprs $N2 j -> val += begin
                eval_state_function(funs[i], series.children[j], args)
            end
        end
        Base.@ncall $N1 SVector x
    end
end

function compute_jacobian2(x, components, args_diff, args_nondiff)
    f = x -> begin
        k = keys(args_diff)
        v = tuple(x...)
        args_diff2 = (; zip(k, v)...)
        args = merge(args_diff2, args_nondiff)
        reduce_state_functions(components.funs, components, args)
    end
    return ForwardDiff.jacobian(x -> f(x), x)
end
