...
@inline function split_composite(composite::NTuple{N, AbstractRheology}, unique_funs_global, funs_local) where {N}
    (expand_composite(composite, unique_funs_global), expand_composite(composite, funs_local))
end
...
function eval_residual3(x, composite, composite_global, state_funs, unique_funs_global, subtractor_vars, inds_x_to_subtractor, inds_args_to_x, args_template, args_all, args_solve)
    subtractor        = generate_subtractor(x, inds_x_to_subtractor)
    args_tmp          = generate_args_from_x(x, inds_args_to_x, args_template, args_all)
    args_solve        = update_args2(args_solve, x)
    subtractor_global = generate_subtractor_global(composite_global, state_funs, unique_funs_global, args_solve)
...
function main(composite, vars, args_solve, args_other)
    ...
    composite_global, composite_local = split_composite(composite, unique_funs_global, funs_local)

    R, J = value_and_jacobian(
        x -> eval_residual3(x, composite_expanded, composite_global, state_funs, unique_funs_global, subtractor_vars, inds_x_to_subtractor, inds_args_to_x, args_template, args_all, args_solve), 
        AutoForwardDiff(), 
        x
    )
    J \ R
...
composite  = viscous1, viscous2, powerlaw
dt         = 1e10
