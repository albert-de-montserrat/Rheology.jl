...
@generated function local_residual_series(  funs_elements::NTuple{N,Any}, 
    ...
    quote
        ...
        Base.@nexprs $N iel -> begin
            ...
            # augmet local arguments with values from x-vector (if applicable)
...
# create a tuple 
@generated function statefuns_to_residual_series(funs_elements::NTuple{N, Any}, n=1) where N
    quote
        @inline
        v = Base.@ntuple $N i -> begin
            val, n = _statefuns_to_residual_series(funs_elements[i],n)
            val
        end
        v
    end
...
@generated function _statefuns_to_residual_series(state_funs::NTuple{N, Function}, n::Int64=1) where N
    quote
        @inline 
        vec = Base.@ntuple $N i -> begin
            v,n = __statefuns_to_residual_series(state_funs[i], n)
            v
        end
        vec, n
    end
...
__statefuns_to_residual_series(::typeof(compute_strain_rate), n) = 1,n
__statefuns_to_residual_series(::typeof(compute_volumetric_strain_rate), n) = 2,n
__statefuns_to_residual_series(::Function, n) = n,n+1



@generated function remove_iteration_vars(args_elements::NTuple{N, NamedTuple}, iteration_vars::NamedTuple) where N
    quote
        @inline 
        vec = Base.@ntuple $N i -> begin
            #v,n = __statefuns_to_residual_series(state_funs[i], n)
...
function main(composite, vars, args_solve0, args_other)
    ...
    funs_elements = series_state_functions_elements(composite)
...
#=                      
    ...
    iteration_vars = (;)
    if any(funs_unique .== compute_strain_rate) #isshear(composite)
    ...
    if any(funs_unique .== compute_volumetric_strain_rate)
    ...
    args_local = remove_iteration_vars(args_elements, iteration_vars) # allocates - to be fixed
=#
